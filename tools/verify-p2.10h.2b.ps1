...
foreach ($needle in @(
  ...
  if ($text -notmatch [regex]::Escape($needle)) {
    Fail "P2.10h.2b incomplet : '$needle' absent"
  }
}

Pass "dialogue quantité présent"
Pass "0 / négatif refusé"
Pass "quantité saisie bornée au stock"
Pass "chemin unloadToActor existant conservé"

Write-Host ""
Write-Host "P2.10h.2b GREEN (structure)" -ForegroundColor Green
